...
- **Post-M22 update (2026-07-25 — `build_challenge` landed):** the M22 note above predates the `build_challenge` pipeline. That pipeline is now **built** (build-time package mirror, launch-time digest-pinning, compose-aware multi-image build + stack launch), and the **joined bundle-launch flow** — the one composite seam M22 left open — is now proven end to end over real PG + Docker, both in-process (`test_joined_e2e_integration`) and over the networked HTTP worker gateway (`test_joined_e2e_http_integration`). Consequently the "one end-to-end organizer + contestant workflow" and internal-alpha exit X1 boxes are now ticked, and the submission-latency ceilings (advisory-lock grain + connection pool) were fixed with the residual in-process p95 shown to be a GIL-bound harness artifact. The remaining unchecked boxes are the genuinely infra/external/at-scale ones (TLS/multi-host deployment, production-scale capacity + ≥99% launch on a real fleet, continuous RPO/PITR, external security review, closed beta). **The OVERALL v1.0 gate is still NOT met.**
- **Post-M22 update (2026-07-26 — reports surface + PITR mechanism + deploy configs):** two more of the residual gaps have moved from "missing artifact" to "environment-bound." (1) The **report surface** (validation / build / eval / competition-run snapshots) is now executed on both planes — JSON API (`test_api_reports_integration`), organizer web pages (`test_web_reports_view_integration`), and the pure-domain contract (`test_reports_domain`) — so "reports surfaced through the UI/API" is ticked. (2) The **continuous-RPO / PITR mechanism** is no longer unconfigured: `deploy/docker-compose.pitr.yml` enables WAL archiving and `scripts/pitr_drill.sh` (+ `test_pitr_drill_integration`) is an executed point-in-time-recovery drill (restore stops exactly at a target time; RPO window ~3–4 s ≤ 300 s; negative control). (3) The **TLS reverse-proxy config** (`deploy/Caddyfile`) and the **control-plane/worker-gateway split** compose (`docker-compose.worker.yml` / `docker-compose.worker-gateway.yml`) now exist. What still blocks v1.0 is unchanged in kind — all of it now needs a **real deployment / real fleet / external party**: standing up the TLS multi-host deployment (which then lets continuous RPO, ≥99%-launch, and sustained 25×20 capacity be *measured*), an external closed beta, and an external security review. **The OVERALL v1.0 gate is still NOT met.**
...
- [ ] Untrusted bundle code (`solver.py`/`healthcheck.py`) runs **only** inside isolated workers, never on host by default (PARTIAL: the **control plane** never executing bundle code is proven by S9, and the distributed-worker bundle-launch flow is now **built + proven** — a worker builds the published bundle's image and launches it in an isolated container, end to end over real PG + Docker via both `test_joined_e2e_integration` and the networked `test_joined_e2e_http_integration`; the residual gap is only the **author-side CLI**, where host execution is still the default with `--sandbox` opt-in)
...
- [x] Reports (validation, scoring, run reports) surfaced through the UI/API — the report surface (validation / build / eval / competition-run snapshots) is now executed on both planes: JSON API (`test_api_reports_integration`), organizer web pages (`test_web_reports_view_integration`), and the pure-domain contract (`test_reports_domain`), all integration-gated + host. Freezes an immutable, append-only snapshot (migration `0017`, `reject_mutation`); reads are pure DB (ADR-001); role-scoped per report (contestant-excluding on version reports, `scoreboard:read` on the run report — the tests assert both the allow and the deny). Scoring/scoreboard surfaces remain covered by `test_web_scoreboard_view_integration`/`test_api_scoreboard_integration`.
...
- [x] One end-to-end organizer + contestant workflow fully wired (highest-priority objective) — the joined flow that **launches the published bundle on a worker** is now one unbroken automated test over real PG + Docker: publish → worker builds the bundle image (`build_challenge`) → launches it as a real container → contestant's seed-derived flag scores → scoreboard, proven both in-process (`test_joined_e2e_integration`) and over the networked HTTP worker gateway (`test_joined_e2e_http_integration`). (Single host + a benign launch command; the **multi-host at-scale, ≥99%-launch** form remains a v1.0 capacity blocker — see `RELEASE_QUALIFICATION.md` §5.)
...
- [ ] >=99% instance launch success; scoreboard update <3s; submission processing <500ms server-side (UNVERIFIED: at the 25×20 target scale in-process submission p95 measured **over target ≈2050 ms** — since fixed at the two product ceilings (lock grain + pool), residual is a GIL-bound harness artifact (`validation/capacity.md`); scoreboard smoke within target; **≥99% launch success at scale has no evidence** — a single launch is proven (joined-flow tests) but needs a real multi-host worker fleet)
...
- [ ] Recovery drill executed: RPO 5min, RTO 30min demonstrated (PARTIAL: **RTO** was rehearsed vs live PG (`recovery_drill.sh` + `test_recovery_drill_integration`); the continuous-**RPO** **mechanism** is now configured (`deploy/docker-compose.pitr.yml` — WAL archiving) and **drilled** end to end (`scripts/pitr_drill.sh` + `test_pitr_drill_integration`: base backup → archive → point-in-time restore to a target time, RPO window ~3–4 s ≤ 300 s, with a negative control). What remains is running the overlay on the real production deployment and measuring continuous **RPO ≤5 min** there — the same production-deployment dimension as the TLS/multi-host blocker)
...
- [ ] **S8** no unrecoverable DB corruption (recovery drill passed) (UNVERIFIED at v1.0: migrations/restore/reconstruction proven (see v0.3) + RTO drill executed; the continuous-**RPO** PITR **mechanism** is now configured + drilled (`scripts/pitr_drill.sh`), but running it on the production deployment to measure continuous RPO there is UNVERIFIED)
...
- [x] One challenge generated, published (immutable/content-addressed), launched on a worker, solved via intended solver, scored, and shown on a scoreboard — end to end — now one unbroken automated flow over real PG + Docker: `test_joined_e2e_integration` (in-process transport) and `test_joined_e2e_http_integration` (over the networked HTTP worker gateway). The worker builds the published bundle's image (`build_challenge`) and launches it as a real container; the intended solver's seed-derived flag scores and lands on the scoreboard. (Single host + a benign launch command for determinism; the multi-host at-scale ≥99%-launch form is the v1.0 capacity blocker, not this criterion.)
...
- [ ] Internal-alpha exit criteria all met (PARTIAL: the internal-alpha exit composite X1 — the joined bundle-launch-on-a-worker flow — is now MET (`test_joined_e2e_integration` + `test_joined_e2e_http_integration`); the only remaining unmet alpha-exit item is the **named internal operators + rollback plan** process sign-off, not an executed artifact)
- [ ] v0.4-beta capabilities complete: admin UI + contestant portal + live ops + reports (PARTIAL: admin UI + contestant portal are exercised by web integration tests, and the full **report surface** is now executed (`test_api_reports_integration`/`test_web_reports_view_integration`/`test_reports_domain`); only "**live ops under load**" remains UNVERIFIED — production-scale — see v0.4-beta)
...
- [ ] Backup/restore tested; RPO/RTO plan in place (**S8**) (PARTIAL: backup/restore round-trip + RTO drill executed (`test_restore_verify_integration`/`recovery_drill.sh`); the continuous-**RPO** PITR plan is now concrete + drilled (`deploy/docker-compose.pitr.yml` + `scripts/pitr_drill.sh`), pending its run on the real deployment)
...
- [ ] >=99% instance launch success; scoreboard update <3s; submission processing <500ms server-side sustained (UNVERIFIED: 25×20 submission p95 measured **over target ≈2050 ms** in-process — since fixed at the two product ceilings, residual is a GIL-bound harness artifact; scoreboard smoke within target; **≥99% launch success at scale has no evidence** — a single launch is proven (joined-flow tests) but needs a real multi-host worker fleet)
...
- [x] Recovery drill (RPO 5min / RTO 30min) rehearsed at least once — `scripts/recovery_drill.sh` + `test_recovery_drill_integration` (RTO measured vs ≤30 min SLO, negative controls) **and** `scripts/pitr_drill.sh` + `test_pitr_drill_integration` (continuous-RPO PITR mechanism: point-in-time restore to a target time, RPO window ~3–4 s ≤ 300 s, negative control) (integration-gated; production-deployment **continuous RPO** measurement pending — same dimension as the TLS/multi-host blocker)
