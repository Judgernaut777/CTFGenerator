...
| Final release qualification (M22) | [../RELEASE_QUALIFICATION.md](../RELEASE_QUALIFICATION.md) | capstone adjudication of every `../RELEASE_CRITERIA.md` gate against the artifacts in this table + the M21 sims | authoritative sign-off pass: S1–S9 QUALIFIED (host / integration-gated), internal-alpha QUALIFIED-by-sim (joined worker-launch seam now closed as one executed flow, in-process + networked), closed-beta PARTIAL; **states plainly that v1.0 is NOT fully release-qualified** with the outstanding deployment / at-scale / external-review blockers |
